...
class Model_dice:
    def __init__(self, values, prob, nb_paths, nb_stocks, nb_dates, payoff):
        ...
        Args:
            values (list): list of possible gamestates of the game 
            prob (list): transition vector 
            nb_paths (int): number of paths generated by the model 
            nb_stocks (int): number of stocks, created by the model
            nb_dates (int): number of dates, where the option can be exercised
            payoff (str): name of the used payoff function 
        """
    ...
    def get_emp_stopping_rule(self, sample):
        ...
        result = (qvalues <= np.array(
            [self.values]*(self.nb_dates), dtype=np.float32))
